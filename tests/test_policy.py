...
def test_sensitive_ticket_overrides_good_retrieval() -> None:
    result = decide(
        analysis(sensitive_topics=["security"]), citation_count=3, best_retrieval_score=0.9
    )
    assert result.outcome == Outcome.REVIEW_REQUIRED


def test_deterministic_privacy_marker_detects_deletion_request() -> None:
    assert set(detect_sensitive_topics("Please delete our data under GDPR.")) == {
        "privacy",
        "account_deletion",
    }
...
def test_escalated_sentiment_requires_human_review() -> None:
    ...
    assert (
        decide(analysis(sentiment="escalated"), citation_count=3, best_retrieval_score=0.9).outcome
...
def test_draft_quality_gate_requires_grounding_and_safety() -> None:
    assert passes_draft_quality(
        DraftQualityAssessment(
            grounding=2, helpfulness=2, tone=1, safety=2, improvement_note="Minor tone edit."
        )
    )
    assert not passes_draft_quality(
        DraftQualityAssessment(
            grounding=1,
            helpfulness=2,
            tone=2,
            safety=2,
            improvement_note="Remove unsupported claim.",
        )
    )
