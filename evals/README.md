...
Fixtures with `"judge_draft": true` opt into the LLM-as-a-judge draft-quality gate. That gate runs by default with `EXECUTION_MODE=ai`; use `--no-judge-drafts` for a routing-and-retrieval-only run. It never changes ticket-processing behavior.
