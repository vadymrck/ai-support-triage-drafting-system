...
class TicketAnalyzer:
    ...
    def _heuristic_analysis(ticket: TicketInput) -> TicketAnalysis:
        ...
        urgency = (
            "critical"
            if any(word in text for word in ("outage", "entire team is blocked", "production down"))
            else "normal"
        )
        issue_type = (
            "access"
            if any(word in text for word in ("sign in", "login", "sso", "password"))
        ...
        return TicketAnalysis(
            ...
            if any(word in text for word in ("frustrated", "unacceptable", "angry"))
            else "neutral",
...
class KnowledgeService:
    ...
    def retrieve(self, session: Session, query: str, limit: int = 3) -> list[Citation]:
        ...
        ranked = sorted(
            (
                (len(query_terms & _terms(row.content)) / max(len(query_terms), 1), row)
                for row in rows
            ),
            reverse=True,
    ...
    def _embed(self, text: str) -> list[float]:
        ...
        client = OpenAI(api_key=self.settings.openai_api_key)
        return (
            client.embeddings.create(model=self.settings.openai_embedding_model, input=text)
            .data[0]
            .embedding
        )
...
def _chunk_text(text: str, max_characters: int = 1_200) -> list[str]:
    return [
        text[index : index + max_characters].strip()
        for index in range(0, len(text), max_characters)
        if text[index : index + max_characters].strip()
    ]


def process_ticket(
    session: Session, ticket: TicketInput, settings: Settings | None = None
) -> ProcessedTicket:
    ...
    analysis.sensitive_topics = sorted(
        set(analysis.sensitive_topics) | set(deterministic_sensitive_topics)
    )
    if has_escalated_sentiment(ticket_text):
        analysis.sentiment = "escalated"
    citations = KnowledgeService(settings).retrieve(
        session, f"{ticket.subject}\n{ticket.description}"
    )
    best_score = citations[0].score if citations else 0.0
    decision = decide(analysis, len(citations), best_score)
    suggested_reply = (
        DraftGenerator(settings).generate(ticket, citations)
        if decision.outcome == Outcome.DRAFT_READY
        else None
    )
...
def _internal_note(
    analysis: TicketAnalysis, citations: list[Citation], decision, suggested_reply: str | None
) -> str:
    citation_lines = (
        "\n".join(f"- {c.document_title}, p. {c.page_number} (score {c.score})" for c in citations)
        or "- No adequate knowledge-base evidence retrieved"
    )
    parts = [
